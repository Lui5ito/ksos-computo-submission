"""Reproduce Tables 2 and 3 from Appendix B of the paper."""
import os
...
def build_model_free(theta_qlow, theta_qupp, lambda1, lambda2):
...
def build_model_same(theta, lambda1, lambda2):
    n_q = len(QUANTILE_LEVELS)
    return Dual_MultiQuantile_KnownMedian(
        quantile_levels=QUANTILE_LEVELS,
        quantile_kernels=[Matern() for _ in range(n_q)],
        quantile_thetas=[theta] * n_q,
        lambda1s=[lambda1] * n_q,
        lambda2s=[lambda2] * n_q,
    )
...
def run_median(X, y, X_test, n_cores):
    """
    For a fixed training data (X,y) and feature vectors in test data X_test,
    run one cross-validation estimation of the hyperparameters for the kernel quantile regression problem,
    generate a LaTeX table of the cross-validation results,
    and return the median predictions at each training and test samples with best hyperparameter combination.
    """
    ...
    gs_median.to_latex_table(
        save_path=OUT_DIR / "multiquant_free_cv_results_median.tex",
        caption="CV results for the median regression.",
        label="tab:cv_multiquantile_median",
        top_k=None,
        param_labels={
            "theta": r"\(\theta^{q_{0.5}}\)",
        },
    ...
    return gs_median.best_model.median, gs_median.best_model.predict(X_test)


def run(X, y, X_test, predictions_median_train, predictions_median_test, params, build_model, n_cores):
    """
    For a fixed training data (X,y) and feature vectors in test data X_test,
    and fixed median prediction at those samples,
    run one cross-validation estimation of the hyperparameters for the multi-quantile regression with known median problem 
    with lengthscales  params,
    generate a LaTeX table of the cross-validation results,
    and return the quantile predictions at each test samples with best hyperparameter combination.
    """
    gs = GridSearch(build_model=build_model, param_grid=params, n_folds=N_FOLDS, n_jobs=n_cores)
    gs.train(
        X=X,
        y=y,
    ...
    print(f"[free] best params: {gs.best_params}")

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    gs.to_latex_table(
        save_path=OUT_DIR / "multiquant_free_cv_results_free.tex",
        caption="CV results for the free lengthscales.",
        label="tab:cv_multiquantile_free",
        top_k=None,
        param_labels={
...
def main(n_cores=1):
    ...
    # Cross-validation for the median
    print(f"\n{'='*70}\n[median]  case={CASE}  solver={SOLVER}  n={N}  \n{'='*70}")
    predictions_median_train, predictions_median_test = run_median(X, y, X_test, n_cores)

    # Cross-validation for both quantile regressions variants
    predictions = {}
    for label, params, build_model in [("same", PARAM_GRID_SAME, build_model_same), ("free", PARAM_GRID_FREE, build_model_free)]:
        print(f"\n{'='*70}\n[{label}]  case={CASE}  solver={SOLVER}  n={N}  \n{'='*70}")
        try:
            predictions[label] = run(X, y, X_test, predictions_median_train, predictions_median_test, params, build_model, n_cores)
        except Exception as e:
            print(f"FAILED [{label}]: case={CASE}  solver={SOLVER}  n={N}    ({e})")

    # Final plot
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    save_path = OUT_DIR / "multiquant_free.pdf"
